...
- [x] 2. 数据模型与数据库迁移
  - [x] 2.1 新增 `ChannelBinding` GORM 模型（server/models/models.go）
  ...
  - [x] 2.2 新增 `CollectEvent` GORM 模型（server/models/models.go）
    - 字段：ID/TenantID/HostID/ChannelID/Type/Message/CreatedAt
    - 引用：design.md「Data Models」
  - [x] 2.3 在 `server/db/db.go` AutoMigrate 列表加入两张新表
    - 引用：design.md「Data Models」
  - [ ]* 2.4 为模型字段验证编写单元测试
...
- [x] 4. 凭据加密服务 CredentialService
  - [x] 4.1 实现 `server/services/credential.go`
...
- [x] 5. Channel 接口与注册表
  - [x] 5.1 定义 `server/collectors/channel.go`
  ...
  - [x] 5.2 实现 `ChannelRegistry`（server/collectors/registry.go）
    - 注册各渠道实现，按类型路由
    - 引用：design.md「Channel 接口」

- [x] 6. SSH 渠道实现 SSHChannel
  - [x] 6.1 实现 `server/collectors/ssh.go`：连接建立
...
- [x] 8. 宝塔面板渠道实现 BaoTaChannel
  ...
  - [x] 8.3 实现宝塔接口结构变更的解析错误处理
    - 解析失败标记 parse_error 并尝试下一渠道
    - 引用：requirements.md Req 7.2
  - [ ]* 8.4 为宝塔渠道编写集成测试
    - httptest 模拟面板响应，验证解析与错误分支

- [x] 9. 采集调度器 CollectorScheduler
  - [x] 9.1 实现 `server/collectors/scheduler.go`：核心调度循环
  ...
  - [x] 9.6 在 server/main.go 启动调度器 goroutine
...
- [x] 11. HTTP API 实现
  - [x] 11.1 新增 `server/controllers/channels.go`
...
- [x] 12. 前端采集渠道管理
  - [x] 12.1 在 HostDetail.jsx 新增「采集渠道」tab
